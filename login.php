<?php 
	...
	$google_client->setRedirectUri('https://jobaskit.com/admin_dashboard/login.php');
	require 'php/messageDisp.php';
	//ob_start();$actual_link = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]";
		...
		if(!isset($token['error'])){
			...
			if(mysqli_num_rows($result) != 1){
				 ...
				 if($query->execute()){
                    ...
                    header('Location: jobaskit.php');
				}
				else echo 'Query not executed, There is some error;';
			}
	    }
	    else{
	        var_dump($token)  ;
...
<html lang="en">
<head>
	<title>Sign-In | Makos InfoTech Pvt Ltd </title>
	<meta charset="UTF-8">
